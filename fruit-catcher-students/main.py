...
# função fintess que avalia o desempenho de uma rede neuronal
def fitness(nn, individual, seed):
    nn.load_weights(individual)
    random.seed(seed)
    return get_score(player=lambda state: nn.forward(state))

# cria uma rede neuronal e calcula o nr de weights necessarios
# guarda os weights da melhor rede em filename
...
# lê os pesos de uma rede neuronal a partir do ficheiro filename
# reconstrói a rede neuronal e devolve uma função que recebe um estado
...
# lê o ficheiro de treino e devolve os nomes das features e os dados (X e Y)
...
# treina uma árvore de decisão com os dados de treino train.csv
# devolve uma função que indica se é fruta ou bomba
